...
def main():
    ...
    while True:
        ...
        print("3) Submit s0 and the next 5 outputs from that stream")
        print("4) Exit")
        choice = input("> ").strip()
        ...
        if choice == "1":
            ...
            print("[info] Recover the transition on basis states, then invert it.")
            print(f"x1 = {x1}")
        elif choice == "3":
            if not leak_revealed:
                print("Leak the stream output first.")
                continue
            submit_solution(seed, expected_outputs)
            return
        elif choice == "4":
            print("Bye.")
            return
